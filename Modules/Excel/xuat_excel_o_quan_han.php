...
if(isset($_POST['xuat_excel_o_quan_han1']) ){
	session_start();
	$objExcel = new PHPExcel;
	$objExcel->setActiveSheetIndex(0);
	$sheet = $objExcel->getActiveSheet()->setTitle();
	$rowCount = 2;
	$sheet->setCellValue('A'.$rowCount,'Danh sách Sinh viên đang ở Ký túc xá quá hạn đến  ngày '.date('d/m/Y') )->mergeCells('A2:'.'N2');
...
//	include 'conn.php';
	...
	$rowCount+=1;
	$sheet->setCellValue('I'.$rowCount,'Vinh, Ngày '.date('d').' tháng ' . date('m').' năm '.date('Y'))->mergeCells('I'.$rowCount.':N'.$rowCount);
	$sheet->getStyle('I'.$rowCount.':N'.$rowCount)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$rowCount+=1;
	...
	$filename = 'danh_sach_sinh_vien_o_ktx_qua_han'.$tenfiel.'.xlsx';
